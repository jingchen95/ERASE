...
extern float power[NUMSOCKETS][XITAO_MAXTHREADS];
#endif
  ...
  //for(auto&& width : ptt_layout[0]){
    for(auto&& elem : partitions) {
...
#ifdef DVFS
#else
      //ptt_val = it->get_timetable(0, width - 1);
      ...
      if (comp_perf < shortest_exec) {
        ...
        new_width = width;
        new_leader = leader;      
        //new_leader = _nthread / new_width *  new_width;
      }
    }
// #ifndef ERASE_target_perf
//   } else { 
...
int PolyTask::globalsearch_Perf(int nthread, PolyTask * it){
  ...
  // Tetralith - CALC: Only train leader core = 0, width = 1,2,4,8,16,32
...
// #endif
      ...
      if (comp_perf < shortest_exec) {
        ...
        new_width = width;
        new_leader = leader;      
        // CALC-Tetralith: after confirming best width, random choose leader core
        //new_leader = rand()%(gotao_nthreads/new_width) * new_width;
      }
#ifdef EAS
    }
#endif
  ...
  for(int a = 0; a < NUMSOCKETS; a++){
    ...
    for(int testwidth = 1; testwidth <= end_idle; testwidth *= 2){
      ...
      for (int h = start_idle; h < end_idle; h += testwidth){
        ptt_value[h] = it->get_timetable(h,testwidth-1); // width = 1
...
int PolyTask::ERASE_Target_Energy_2(int nthread, PolyTask * it){
  ...
  ptt_full = true;
#ifdef DynaDVFS
  if(current_freq == 2035200){
    env = 0; 
  }else{
    if(current_freq == 345600){
      env = 3;
    }
  }
#ifdef DEBUG
  LOCK_ACQUIRE(output_lck);
  std::cout << "[DEBUG] For task " << it->taskid << ", PTT has been fully trained. Power Model uses env = " << env << std::endl;
  LOCK_RELEASE(output_lck);
#endif  
//   if(freq_dec > 0){
//     env = 3;
...
//   }
  ...
  float shortest_exec = 100000.0f;
  for(auto&& width : ptt_layout[0]) {
    dyna_P = power[0][width];
    auto&& ptt_val = 0.0f;
    ptt_val = it->get_timetable(0, width - 1);
    comp_perf = ptt_val * (dyna_P + idleP*(width/COREPERSOCKET));
    if (comp_perf < shortest_exec) {
      shortest_exec = comp_perf;
      it->width = width;   
    }
#ifdef DEBUG
    LOCK_ACQUIRE(output_lck);
    std::cout << "[Tetralith-ERASE] For task " << it->taskid << ", PTT ful. Width = " << width << ". IdlePower: " << idleP << ", dynaPower: " << dyna_P << ". ptt_val = " << ptt_val << ". Energy Prediction: " << comp_perf << std::endl;
    LOCK_RELEASE(output_lck);
#endif  
  }
  // ERASE-Tetralith: after confirming best width, random choose leader core
  it->leader = rand()%(gotao_nthreads/it->width) * it->width;
#ifdef DEBUG
  LOCK_ACQUIRE(output_lck);
  std::cout << "[Tetralith-ERASE] For task " << it->taskid << ", Best Width = " << it->width << ". Leader core = " << it->leader << std::endl;
  LOCK_RELEASE(output_lck);
#endif  
#endif
...
#ifdef DEBUG
      LOCK_ACQUIRE(output_lck);
      std::cout << "Average[" << a << "][" << testwidth << "] = " << average[a][testwidth] << ", idle power = " << idleP << ", dynamic power = " << dyna_P << ", energy prediction = " << comp_perf << std::endl;
      LOCK_RELEASE(output_lck);
#endif
      if (comp_perf < shortest_exec) {
        shortest_exec = comp_perf;
...
#ifdef DEBUG
              LOCK_ACQUIRE(output_lck);
              std::cout <<"[CATS] MIN & MAX, Priority=1, task "<< (*it)->taskid <<" is pushed to WSQ of thread "<< i << std::endl;
              LOCK_RELEASE(output_lck);
#endif
            }else{
              int i = rand() % 2;
...
#ifdef DEBUG
              LOCK_ACQUIRE(output_lck);
              std::cout <<"[CATS] MIN & MAX, Priority=0, task "<< (*it)->taskid <<" is pushed to WSQ of thread "<< i << std::endl;
              LOCK_RELEASE(output_lck);
#endif
            }else{
              int i = 2 + rand() % 4;
...
// #ifdef DEBUG
//             LOCK_ACQUIRE(output_lck);
//             std::cout <<"[DEBUG] Task "<< (*it)->taskid <<" is pushed into thread "<< (*it)->leader << "'s queue." << std::endl;
//             LOCK_RELEASE(output_lck);
// #endif
          } 

          // D_give_A = 1;
          // if(std::distance(out.begin(), it) >= out.size() - D_give_A){
          //   (*it)->leader = 2;   
          //   (*it)->width =  4;
          // }else{
          //   (*it)->leader = 0;
